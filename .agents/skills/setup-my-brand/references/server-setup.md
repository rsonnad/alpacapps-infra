...
5. Append to CLAUDE.local.md:
   ...
   - **If SSH uses password auth** (no key): use password file — document `sshpass -f ~/.ssh/my-brand-<service>.pass ssh ...` and one-time setup: `echo 'BASE64' | base64 -d > ~/.ssh/my-brand-<service>.pass && chmod 600 ~/.ssh/my-brand-<service>.pass`. Store the password base64 in CLAUDE.local.md so the password never appears on the command line (see "Passwords and shell escaping" in initial scaffold).
...
8. Append to CLAUDE.local.md: login credentials, Cloud Account Name, console URL, IP, SSH command. **If the instance uses password auth** (e.g. before key is set): use password file and base64 as in "Passwords and shell escaping"; document `sshpass -f ~/.ssh/my-brand-oracle.pass ssh ...` and one-time file setup.
