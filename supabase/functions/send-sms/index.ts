...
function getSmsBody(type: SmsType, data: Record<string, any>): string {
  switch (type) {
    case "payment_reminder":
      return `Hi ${data.first_name}, friendly reminder: your ${data.period || 'rent'} of $${data.amount} is due ${data.due_date}. Pay via Venmo @Propertyproperty or Zelle admin@YOUR_DOMAIN - YOUR_PROPERTY_NAME`;
    ...
    case "deposit_requested":
      return `Hi ${data.first_name}, your deposit of $${data.total_due} is due${data.due_date ? ` by ${data.due_date}` : ''}. Pay via Venmo @Propertyproperty or Zelle admin@YOUR_DOMAIN - YOUR_PROPERTY_NAME`;
    ...
    case "lease_sent":
      return `Hi ${data.first_name}, your lease agreement has been sent for e-signature. Please check your email from SignWell and sign at your earliest convenience. - YOUR_PROPERTY_NAME`;
